...
unsafe extern "C" fn read_delta(info: duckdb_function_info, output: duckdb_data_chunk) {
    ...
    let bind_data = info.get_bind_data() as *const MyBindDataStruct;
    ...
    (*init_data).done = true;
    output.set_size(final_row as u64);
}

unsafe fn set_bytes(output: &DataChunk, final_row: usize, idx: usize, bytes: &[u8]) {
    let cs = CString::new(bytes).unwrap();

    let result_vector = output.get_vector(idx as u64);

    result_vector.assign_string_element_len(final_row as u64, cs.as_ptr(), bytes.len() as u64);
}

unsafe fn assign<T: 'static>(output: &DataChunk, final_row: usize, idx: usize, v: T) {
